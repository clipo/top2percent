...
- H2 (Books): Cliff's delta, Pearson r, p-value
...
def analyze_replicate(replicate_num, df):
    ...
    df_matched = df[df['openalex_found'] is True].copy()
    ...
    if n_matched < 50:
        print("WARNING: Too few matches for reliable analysis")
        return None
    ...
    if len(high_books) > 5 and len(low_books) > 5:
        ...
        print(f"  Cliff's delta: {results['h2_cliffs_delta']:.3f}")
...
def main():
    """Analyze all replicates and generate summary statistics."""
    print("=" * 80)
    ...
    print(
        f"\n{'Replicate':<12} {'H1_d':<8} {'H1_r':<8} {'H1_p':<10} "
        f"{'H2_d':<8} {'H2_r':<8} {'H2_p':<10}"
    )
    print("-" * 80)
    ...
    for _, row in results_df.iterrows():
        ...
        p_str1 = "<0.001" if h1_p < 0.001 else f"{h1_p:.4f}"
        p_str2 = "<0.001" if h2_p < 0.001 else f"{h2_p:.4f}"
    ...
    print(
        f"{'Mean +/- SD':<12} {h1_d_mean:.2f}+/-{h1_d_std:.2f} "
        f"{h1_r_mean:.2f}+/-{h1_r_std:.2f} {'ALL':<9} "
        f"{h2_d_mean:.2f}+/-{h2_d_std:.2f} "
        f"{h2_r_mean:.2f}+/-{h2_r_std:.2f} {'ALL':<9}"
    )
    ...
    for col in ['h1_pvalue', 'h2_pvalue', 'h3_pvalue', 'h4_pvalue']:
        if col in results_df.columns:
            ...
            if max_p >= 0.001:
                all_sig = False
                print(f"\nWARNING: {col}: max p-value = {max_p:.4f}")
