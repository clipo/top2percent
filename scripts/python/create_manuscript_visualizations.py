...
is_scopus_only = df_both['authfull'].isin(top_2_scopus) & ~df_both['authfull'].isin(top_2_openalex)
df_both.loc[is_scopus_only, 'category'] = 'Scopus Top 2% Only'
...
for name, text_offset in specific_researchers.items():
    ...
    if len(match) > 0:
        row = match.iloc[0]
        ax.scatter(
            row['rank_global'], row['rank_openalex'],
        ...
        ax.annotate(
            ...
            bbox=dict(
                boxstyle='round,pad=0.25', facecolor='yellow',
...
ax.text(
    ...
    fontsize=6, fontweight='bold',
    bbox=dict(
        boxstyle='round,pad=0.3', facecolor='yellow',
        alpha=0.8, edgecolor='black', linewidth=0.5,
    ),
    zorder=10,
)
...
ax.set_title(
    'Comparison of Scopus vs OpenAlex-Based Rankings\n(n=537 researchers)',
...
ax.text(
    0.95, 0.05, f'Spearman rho = {corr:.3f}',
...
ax1.axvline(
    rank_changes.median(), color='red', linestyle='--',
    linewidth=2, label=f'Median: {rank_changes.median():.0f}',
)
...
for i, (idx, row) in enumerate(field_stats.iterrows()):
    ax2.text(
        row['median'] + 1000, i, f"n={int(row['count'])}",
...
for idx, (_, row) in enumerate(top_2_df.iterrows(), 1):
    ...
    latex_table += (
        f"{name} & {field} & {rank_oa} & {rank_sc} {change_str} "
        f"& {c_score:.2f} & {citations} & {elsevier_pct:.1f}\\% \\\\\n"
    )
...
max_change_idx = df_both['rank_change'].idxmax()
max_change_name = df_both.loc[max_change_idx, 'authfull']
summary_stats = pd.DataFrame({
    ...
    'Value': [
        ...
        f"{df_both['rank_change'].max():,.0f} ({max_change_name})",
        f"{corr:.3f}",
        "",
        "",
...
extreme_cols = [
    'authfull', 'field', 'rank_openalex',
    'rank_global', 'rank_change',
    'c_score_openalex', 'elsevier_pct',
    'nc', 'h',
]
extreme_df = df_both.nlargest(10, 'rank_change')[extreme_cols]
...
els_diff = df_both['elsevier_pct'].mean() - top_2_df['elsevier_pct'].mean()
print(f"Difference: {els_diff:.1f} percentage points")
