...
def calculate_composite_score(metrics, max_values):
    """Calculate c-score using the Ioannidis composite formula."""
    score = 0
...
if only_openalex:
    print(f"\n{'=' * 80}")
    print("GAINED TOP 2% STATUS (in OpenAlex, not Scopus):")
    print("=" * 80)
...
print(
    f"  High-Elsevier in top 2%: {high_elsevier_in_top}/{len(top_2_df)} "
    f"({high_elsevier_in_top / len(top_2_df) * 100:.1f}%)"
)
...
print("Comparison table: scopus_vs_openalex_rankings.csv")
